var Battle = function(roomid, type, nojoin) {
	...
	insides += '<img src="http://play.pokemonshowdown.com/sprites/bw/0.png" id="' + roomid + 'oppmon" class="oppmon" />';
	insides += '<img src="http://play.pokemonshowdown.com/sprites/bw/0.png" id="' + roomid + 'youmon" class="youmon" />';
	insides += '</div>';
	insides += '</div>';
	...
	if ($("body").width() > 550 * 1.5 && $("body").height() > 330 * 1.5) {
		...
		$('#' + roomid + 'battle').width(550).height(330);
		...
		for (var currentTime = this.currentTime + 1; currentTime < this.events; currentTime++) {
			...
			*/
			...
			if (current.event == "request" && this.battle.active) {
				current.updateIcons = true;
				var insides = '';
				for (var i in this.battle.active[0].moves) {
					var disabled = "";
					if (this.battle.active[0].moves[i].disabled) disabled = " disabled='true'";
					var type = exports.BattleMovedex[this.battle.active[0].moves[i].id].type;
					insides += '<button' + disabled + ' id="' + this.battle.active[0].moves[i].move + '---' + roomid + '" class="type-' + type + '"><div><span class="movetitlebutton">' + this.battle.active[0].moves[i].move + '</span></div><div><span class="left">' + type + '</span><span class="right">' + this.battle.active[0].moves[i].pp + '/' + this.battle.active[0].moves[i].maxpp + '</span></div></button>';
				}
			...
			if (current.updateIcons || current.event == "start" || current.event == "teampreview") {
				...
				for (var i in user.mons) {
					...
					insides += '<span class="col iconcol' + dead + '" id="' + ((!user.mons[i].mon && Math.floor(user.mons[i].mon) !== 0) ? mon : user.mons[i].mon) + '-' + roomid + '" style="width: 32px;height: 24px;' + Tools.getIcon(info) + '"></span>'
					mon++;
				}
